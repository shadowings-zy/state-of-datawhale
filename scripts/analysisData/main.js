...
const joinRepoDetailByName = (repoList) => {
    return repoList.map(item => {
        ...
        const repoDataDetailListFile = fs.readFileSync(path.join(__dirname, `../data/repoStarDetail.json`), 'utf8')
        const repoDataDetailList = JSON.parse(repoDataDetailListFile)
        const repoDataDetail = repoDataDetailList.find(item => item.repoName === filename)
        
        const monthlyStars = repoDataDetail.monthlyStars
        const monthlyTotalStars = repoDataDetail.monthlyTotalStars
        return {
            name: filename,
...
const getRepoStarMoreThan1000 = () => {
    const data = fs.readFileSync(path.join(__dirname, `../data/${CURRENT_KEY}/repoList.json`), 'utf8')
